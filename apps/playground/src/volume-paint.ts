...
import {createVolume,getVolume,registerVolume,blendVolumePaint,volumePaintSchema,volumeDocSchema,type SceneDoc,type VolumeDoc,type VolumePaint} from '@hitreg/core';
import {SPLAT_ATTRIBUTES} from '@hitreg/render';

/** Paint-only editor: the native volume remains the authoring source; geometry is untouched. */
export function mountVolumePaint(opts:{canvas:HTMLCanvasElement;camera:THREE.PerspectiveCamera;doc:()=>SceneDoc;objects:()=>Map<string,THREE.Object3D>;editing:()=>boolean}){
 ...
 function stroke(e:PointerEvent){if(!session||!root)return;const radius=Number(inputs[0]!.value),strength=Number(inputs[1]!.value);if(!Number.isFinite(radius)||radius<=0||radius>30)return;const r=opts.canvas.getBoundingClientRect();ray.setFromCamera(new THREE.Vector2((e.clientX-r.left)/r.width*2-1,1-(e.clientY-r.top)/r.height*2),opts.camera);const hit=ray.intersectObject(root,true)[0];if(!hit)return;const p=root.worldToLocal(hit.point.clone());if(lastPoint&&p.distanceTo(lastPoint)<radius*.2)return;const parsedPaint=volumePaintSchema.safeParse({id:crypto.randomUUID(),center:p.toArray(),radius,strength,layer:Number(select.value),fill:mode.value==='fill',maxAngle:mode.value==='fill'?Number(inputs[2]!.value):180,...(mode.value==='fill'&&hit.face?{normal:hit.face.normal.toArray()}: {})});if(!parsedPaint.success){status.textContent='Check radius, strength and angle';return;}const paint=parsedPaint.data;if(!dragging){session.undo.push(structuredClone(session.doc.paint));session.redo=[];dragging=true;}lastPoint=p;session.doc.paint.push(paint);session.dirty=true;apply(paint);status.textContent='Unsaved paint · '+session.doc.paint.length+' dabs';}
 const down=(e:PointerEvent)=>{if(e.button!==0||!e.shiftKey||!panel.open||panel.hidden)return;e.preventDefault();e.stopImmediatePropagation();lastPoint=undefined;stroke(e);};const move=(e:PointerEvent)=>{if(!dragging)return;e.preventDefault();e.stopImmediatePropagation();if(mode.value!=='fill')stroke(e);};const up=()=>{dragging=false;lastPoint=undefined;};opts.canvas.addEventListener('pointerdown',down,true);opts.canvas.addEventListener('pointermove',move,true);window.addEventListener('pointerup',up,true);
 buttons[0]!.onclick=()=>{if(!session||!session.undo.length)return;session.redo.push(structuredClone(session.doc.paint));session.doc.paint=session.undo.pop()!;session.dirty=true;repaint();status.textContent='Undone · unsaved';};buttons[1]!.onclick=()=>{if(!session||!session.redo.length)return;session.undo.push(structuredClone(session.doc.paint));session.doc.paint=session.redo.pop()!;session.dirty=true;repaint();status.textContent='Redone · unsaved';};
 buttons[2]!.onclick=async()=>{if(!session)return;const target=session,id=activeId,snapshot=volumeDocSchema.parse(target.doc);status.textContent='Saving…';try{const read=await fetch('/__hitreg/asset-file?file='+encodeURIComponent('volumes/'+id+'.json'));if(!read.ok)throw Error('Could not read the current volume');const fresh=volumeDocSchema.parse(await read.json());if(JSON.stringify(fresh.paint)!==target.baseline)throw Error('Paint changed on disk. Reload before saving to avoid overwriting it.');const content=volumeDocSchema.parse({...fresh,paint:snapshot.paint});const res=await fetch('/__hitreg/write-asset',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({file:'volumes/'+id+'.json',content:JSON.stringify(content,null,2)})});if(!res.ok)throw Error('Save failed: '+res.status);registerVolume(id,content);target.baseline=JSON.stringify(snapshot.paint);if(JSON.stringify(target.doc.paint)===JSON.stringify(snapshot.paint))target.dirty=false;status.textContent=target.dirty?'Saved snapshot · newer paint unsaved':'Paint saved';}catch(e){status.textContent=String(e);}};
 const timer=window.setInterval(refresh,700);refresh();return()=>{clearInterval(timer);panel.remove();opts.canvas.removeEventListener('pointerdown',down,true);opts.canvas.removeEventListener('pointermove',move,true);window.removeEventListener('pointerup',up,true);};
}
