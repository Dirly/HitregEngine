...
- `script` — `{ name, params }`: attach registered behaviors (spinner, oscillator, platform-mover (ping-pong platform: distance/speed/dwell), door (opens by proximity of openerTag within range — slides by move and/or spins rotateY deg), face-target (yaws to face the nearest targetTag entity; turnSpeed 0 snaps, else eases; range 0 = unlimited — turrets/cameras/NPCs), damageable (hp that drops on collision with a hazardTag collider, invulnMs i-frames, drives its health billboard, hides at 0 — local/single-player hazard primitive), player-controller, collectible, anim-cycler + project scripts in src/scripts/). Script context: setAnimation(clip, fade, { loop }) — `loop: false` plays a clip once and emits the local `animation.completed` event ({ entityId, clip }) at the end (attack/emote → idle chaining), playSound(id?), setActiveCamera(id), viewForward(), sim velocity APIs, `ctx.after(seconds, cb)` / `ctx.every(seconds, cb)` — deterministic sim-stepped timers (replay/multiplayer-safe, NOT setTimeout; each returns a cancel fn, all auto-cancelled when the script disposes/suspends), and `ctx.playerData` — experience-scoped persistence (`get/set/increment/transaction/keys(namespace, ...)`, async, quota+rate-limited, atomic revisions; survives across sessions — e.g. `ctx.playerData?.increment("stats", "sessions")`). See `session-counter` for the minimal example.
