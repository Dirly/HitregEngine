...
Task-specific reference moved from `CLAUDE.md`. The repository entrypoints carry
shared invariants and the reading route; read only the named subsystem relevant
to the current task here. Paths and commands are relative to the repository or
the working directory stated by the command, as in the original instructions.
Use the running server's actual port in place of the example port 5173.

To locate a subsystem without loading the whole reference:

```powershell
rg -n '^\- \*\*' docs/agent-workflow.md
```
...
- **Third-person camera** (`ThirdPersonCameraRig` in `@hitreg/render`): ONE
  ...
  not a wall). It is an MMO camera: the boom only SHORTENS (nothing but the
  mouse pitches it), a look-ahead closes it before a doorway has to, the wheel
  runs into first person, and the body is hidden rather than rendered from the
  inside. Judgment + the traps (collision and framing must share ONE origin;
...
- **Spells and VFX** (generated, not authored): a `spell` data asset
  ...
  Save). **Environmental fire** (torches, braziers, campfires) is the same
  system standing still: a `vfx` component (`{ effect, material }`) plays a
  `vfx` asset forever on its entity, coloured by a MATERIAL's color/emissive,
  its light in the scene light budget; `node tools/fx.mjs fire <project>`
  installs the standard set (`env/fire-*`, `fx/fire*`). Sprites are picked by ROLE from a project catalog, never by name —
